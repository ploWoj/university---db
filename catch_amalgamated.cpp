...
TestCaseProperties& operator|=(TestCaseProperties& lhs,
                               TestCaseProperties rhs) {
  lhs = static_cast<TestCaseProperties>(static_cast<TCP_underlying_type>(lhs) |
                                        static_cast<TCP_underlying_type>(rhs));
  return lhs;
}
...
void TestCaseInfo::internalAppendTag(StringRef tagStr) {
  ...
  backingLCaseTags += ']';
  tags.emplace_back(
      StringRef(backingTags.c_str() + backingStart, backingEnd - backingStart),
...
IConfig const* ReporterConfig::fullConfig() const {
  return m_fullConfig;
}
...
void IStreamingReporter::listReporters(
  ...
  for (auto const& desc : descriptions) {
    if (config.verbosity() == Verbosity::Quiet) {
      Catch::cout() << TextFlow::Column(desc.name).indent(2).width(5 +
                                                                   maxNameLen)
                    << '\n';
    } else {
      Catch::cout()
          << TextFlow::Column(desc.name + ":").indent(2).width(5 + maxNameLen) +
...
size_t ParserBase::cardinality() const {
  return 1;
}
...
bool BoundRef::isContainer() const {
  return false;
}

bool BoundRef::isFlag() const {
  return false;
}

bool BoundFlagRefBase::isFlag() const {
  return true;
}

}  // namespace Detail

Detail::InternalParseResult Arg::parse(
    std::string const&,
...
Detail::InternalParseResult Opt::parse(
    std::string const&,
    Detail::TokenStream const& tokens) const {
  auto validationResult = validate();
  if (!validationResult)
...
#else
    if (name[0] != '-')
      return Detail::Result::logicError("Option name must begin with '-'");
#endif
  }
...
void listTags(IStreamingReporter& reporter, IConfig const& config) {
  ...
  for (auto const& testCase : matchedTestCases) {
    for (auto const& tagName : testCase.getTestCaseInfo().tags) {
      ...
      if (it == tagCounts.end())
        it = tagCounts.insert(std::make_pair(tagName.lowerCased, TagInfo()))
                 .first;
...
std::vector<TestCaseHandle> const& getAllTestCasesSorted(
    IConfig const& config) {
...
Column::iterator& Column::iterator::operator++() {
  ...
  if (m_pos < current_line.size() && current_line[m_pos] == '\n') {
    ...
    while (m_pos < current_line.size() && isWhitespace(current_line[m_pos])) {
      ++m_pos;
    }
  }
...
EndsWithMatcher EndsWith(std::string const& str,
                         CaseSensitive caseSensitivity) {
...
void handleExceptionMatchExpr(AssertionHandler& handler,
  ...
  MatchExpr<std::string, StringMatcher const&> expr(std::move(exceptionMessage),
...
void SonarQubeReporter::writeTestFile(
    ...
    TestGroupNode::ChildNodes const& testCaseNodes) {
...
class TapAssertionPrinter {
 public:
  ...
  TapAssertionPrinter(std::ostream& _stream,
                      AssertionStats const& _stats,
                      std::size_t _counter)
      : stream(_stream),
        result(_stats.assertionResult),
...
// if string has a : in first line will set indent to follow it on
// subsequent lines
void printHeaderString(std::ostream& os,
