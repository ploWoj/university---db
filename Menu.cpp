...
void Menu::printMainManu() const {
  ...
  std::cout << std::setw(itemSize) << "[ 1] Print DB" << std::setw(itemSize)
            ...
            << "[10] Remove Person" << std::setw(itemSize) << "[ 0] Quit"
            << '\n'
            << std::setw(itemSize) << "[ 2] Add Student" << std::setw(itemSize)
            << "[ 5] Sort by surnname" << std::setw(itemSize)
            << "[ 8] Find surnname" << std::setw(itemSize)
            << "[11] Save to file" << '\n'
            << std::setw(itemSize) << "[ 3] Add Emplyee" << std::setw(itemSize)
...
void Menu::separator() {
  std::cout << std::right << std ::setfill('-')
            << std::setw(typeColumnWidth + firstNameColumnWidth +
                         lastNameColumnWidth + peselColumnWidth +
                         addressColumnWidth + sexColumnWidth +
                         indexNumberColumnWidth + salaryColumnWidth)
            << '\n';
...
void Menu::printPanel(const std::string message) {
  separator();
  std::cout << "Database size: " << db_.getVector().size()
            << " | message: " << message << '\n';
  separator();
}
...
std::string Menu::menuFindByPesel() {
  ...
  if (db_.validationByPesel(pesel)) {
    ...
    if (find) {
      return "A person with the pesel " + pesel + ", has been found " +
             find->getName() + " " + find->getSurname() + ".";
    }
