...
void University::sortBySalary() {
  std::sort(university_.begin(), university_.end(),
            [](const std::unique_ptr<Person>& lhsPtr,
               const std::unique_ptr<Person>& rhsPtr) {
              return getSalaryIfIs(lhsPtr).value_or(0.0) > getSalaryIfIs(rhsPtr).value_or(0.0) ||
                     (getSalaryIfIs(lhsPtr).value_or(0.0) == getSalaryIfIs(rhsPtr).value_or(0.0) &&
                      lhsPtr->getSurname() < rhsPtr->getSurname());
            });
}
